...
function displayFilmImages(moviesList) {
  ...
  const cleanedMovieList = moviesList.results.map((movie) => {
    return {
      title: movie.title,
      poster_url: movie.poster_path ? `${IMAGE_BASE_URL}/w400${movie.poster_path}` : 'https_placehold.co/400x600/0d1117/aaaaaa?text=Sem+Imagem',
    };
  });
  ...
  cleanedMovieList.forEach((movie) => {
    ...
    filmImageElement.onerror = (e) => { e.target.src = 'https_placehold.co/400x600/0d1117/aaaaaa?text=Erro+Img'; };
